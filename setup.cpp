...
public:
    cIptvMenuInfo();
    ~cIptvMenuInfo() override;
    void Display() override;
    eOSState ProcessKey(eKeys keyP) override;
};

cIptvMenuInfo::cIptvMenuInfo() : cOsdMenu(tr("IPTV Information")), textM(""), timeoutM(), pageM(IPTV_DEVICE_INFO_GENERAL) {
...
cIptvMenuInfo::~cIptvMenuInfo() = default;
...
eOSState cIptvMenuInfo::ProcessKey(eKeys keyP) {
    ...
    case kRight:
        ...
        cStatus::MsgOsdTextItem(nullptr, NORMALKEY(keyP)==kUp || NORMALKEY(keyP)==kLeft);
...
cIptvPluginSetup::cIptvPluginSetup() {
    ...
    SetHelp(nullptr, nullptr, nullptr, trVDR("Button$Info"));
}

void cIptvPluginSetup::Setup() {
    int current = Current();
    ...
    if (sectionFilteringM) {
        ...
        for (int i = 0; i < numDisabledFiltersM; ++i) {
            Add(new cMenuEditStraItem(*cString::sprintf(" %s %d", tr("Filter"), i + 1),
                                      &disabledFilterIndexesM[i],
                                      SECTION_FILTER_TABLE_SIZE,
                                      disabledFilterNamesM));
...
eOSState cIptvPluginSetup::ShowInfo() {
    debug1("%s", __PRETTY_FUNCTION__);
        ...
        case kInfo:
            if (Current() < helpM.Size())
                return AddSubMenu(new cMenuText(cString::sprintf("%s - %s '%s'",
                                                                 tr("Help"),
                                                                 trVDR("Plugin"),
                                                                 PLUGIN_NAME_I18N), helpM[Current()]));
        default:
            state = osContinue;
            break;
        }
